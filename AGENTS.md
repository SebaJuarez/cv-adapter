...
- **Fase 6 — Generación asistida inline** ✅ (6.6) (`f27ac04`, e implementación en `d00c5c1`→`e3304f9`): botón "Generar versión" en el bullet del target — aparece SIEMPRE en bullets con logro (regla post-QA 12-ago-2026, spec en `docs/specs/iteracion_post_qa_f6.md`): con ángulo preferido de la selección se orienta a ese; sin ángulo preferido genera variante **genérica** (`angle: ""`, nunca inventa ángulo); opción "Generar otra redacción…" en el popover F3 (existe desde 1 variante approved) con los 9 ángulos; modal de comparación lado a lado con términos no verificados contra el logro resaltados (`generate_variant_text` en `llm_node.py` + `POST /api/variants/generate` en `api/routers/variants.py`, payload con facts del frontend, 422 ángulo inválido no vacío / 502 proveedor — `""` = genérica válida). **Decisión de API key cerrada**: funciona con el proveedor activo (Ollama local incluido); proveedor caído → toast con enlace a Configuración. "Usar y guardar" agrega la variante `approved`/`generated` al master en memoria (persiste con el POST `/api/master-cv`); "Usar solo esta vez" no toca el master. **QA manual completado (12-ago-2026)**: modal F6, popover F3 y PDF verificados end-to-end con OpenRouter — fix aplicado: timeout de `generate_variant_text` 30s→90s (`llm_node.py`, la cola del free tier daba 502 engañoso). **Iteración post-QA (12-ago-2026)**: ✏/⇄ siempre + ángulo genérico, 422 de ángulo inválido, y proveedor **OpenRouter** en Configuración (preset UI que guarda `llm_provider: openai` + `openai_base_url` de OpenRouter — el despacho `_call_llm` no cambia; campos condicionales por proveedor y sugerencias vía datalist en `settings.js`). Hallazgos y mejoras candidatas en `docs/qa_f6_hallazgos.md`.
- **Fase 7 — Loop de feedback en historial** ✅ (6.7): `merge.py` persiste en la metadata por bullet (`_src_variant_map`) el ángulo y texto emitidos, y `extract_bullet_variants` reconstruye la traza en orden efectivo; `add_run` la guarda como `bullet_variants` por corrida (solo si hay logros con variantes — runs legacy sin clave), con el texto incluido para que el historial siga legible si la variante se deprecia o se borra del master. En el detalle de corrida (tab Análisis) se muestra "Variantes usadas en esta corrida" (texto + ángulo + ID); en Historial, la sección "Tus variantes más usadas" (`GET /api/history/stats/variants` → `aggregate_variant_stats` en `src/history.py`): N corridas por variante y cuántas llegaron a entrevista u oferta (`SUCCESS_STATUSES`).
