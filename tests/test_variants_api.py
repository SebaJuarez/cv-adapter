...
valida datos mínimos (422), propaga el fallo del proveedor como 502 con
detalle legible y devuelve el texto + términos no verificados.
"""
import pytest
...
def test_angulo_vacio_422(client):
    res = client.post(
        "/api/variants/generate",
        json=_payload(angle="  "),
    )
    assert res.status_code == 422
    assert "Falta el ángulo" in res.json()["detail"]
